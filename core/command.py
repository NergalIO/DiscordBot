...
class Command:
    def __init__(self, name: str, description: str, command: types.FunctionType, args: dict[str, type]) -> None:
        self.name = name
    ...
    def __call__(self, **kwargs) -> None:
        if len(kwargs.keys()) != len(self.args.keys()):
            ...
            raise CommandArgumentError(f"Not found a few arguments for command <{self.name}> : {", ".join(missed_args)}")
        for key, value in kwargs.items():
            if self.args[key] != type(value):
                raise CommandArgumentError(f"Argument <{key}> must be {self.args[key]} type, not {type(value)}!")
        self.command(**kwargs)
